      ...
      const _bindInterval = setInterval(function() {
          ...
          if (selectMesin.tomselect) {
              ...
              if (!selectMesin.value) {
                  ...
                  const _draftKey = `autosave_mtce_<?= esc($departemenSlug ?? '') ?>_<?= esc($jenisSlug ?? '') ?>_<?= esc($categorySlug ?? '') ?>_${currentUserId}`;
                  try {
                      ...
                      if (_saved) {
                          ...
                          if (_dd['id_mesin'] && _lastDate === _today && _gapMinutes <= 10) {
                              ...
                              updateFields();
                              setTimeout(function() { window._loadingDraft = false; }, 600);
                          }
                      }
                  } catch(e) {}
              } else {
          ...
          } else if (_bindAttempts > 30) { // max 3 detik (30 x 100ms)
              clearInterval(_bindInterval);
              selectMesin.addEventListener('change', function() {
                  updateFields();
                  checkDuplicateOnChange();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getFreshServerTime() {
        const d = new Date();
        return {
            raw: d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0') + '-' + String(d.getDate()).padStart(2,'0') + ' ' + String(d.getHours()).padStart(2,'0') + ':' + String(d.getMinutes()).padStart(2,'0') + ':' + String(d.getSeconds()).padStart(2,'0'),
            display: String(d.getDate()).padStart(2,'0') + '/' + String(d.getMonth()+1).padStart(2,'0') + '/' + d.getFullYear() + ' ' + String(d.getHours()).padStart(2,'0') + ':' + String(d.getMinutes()).padStart(2,'0') + ':' + String(d.getSeconds()).padStart(2,'0')
        };
    }

    window.clearFormAndLoadDraft = function() {
        const isEditChk = <?= isset($isEdit) && $isEdit ? 'true' : 'false' ?>;
        if (isEditChk) return;
        ...
        // Reset waktu mulai
        const freshTime = getFreshServerTime();
        const displayWaktu = document.getElementById('displayWaktuMulai');
    ...
    function getStorageKey() {
        return `autosave_mtce_${departemenSlug}_${jenisSlug}_${categorySlug}_${currentUserId}`;
    }
        ...
        if (saved) {
            try {
                ...
                for (let name in data) {
                    ...
                    if (name === 'waktu_mulai') {
                        if (gapMinutes <= 10) {
                            ...
                            if (displayEl) {
                                let rawDate = new Date(data[name].replace(/-/g, '/'));
                                if (!isNaN(rawDate)) {
                                    displayEl.value = String(rawDate.getDate()).padStart(2,'0') + '/'
                ...
                if (mesinSelect && mesinSelect.tomselect && data["id_mesin"]) {
                    if (!urlIdMesin) {
                        let rMesinId = Array.isArray(data["id_mesin"]) ? data["id_mesin"][0] : data["id_mesin"];
                        mesinSelect.tomselect.setValue(rMesinId, true);
                    }
                }
                ...
                setTimeout(function() { window._loadingDraft = false; }, 400);

                // Pulihkan tampilan foto abnormal untuk radio Δ yang aktif
                document.querySelectorAll('.hasil-check-radio:checked').forEach(function(radio) {
                    if (radio.value === 'Δ') {
                        const paramId = radio.getAttribute('data-param-id');
                        const wrap = document.getElementById('foto-wrap-' + paramId);
                        const fi = document.getElementById('foto-input-1-' + paramId);
    ...
    setTimeout(function() {
        loadFormData();
        // Simpan segera agar waktu_mulai server masuk ke draf walau belum ada interaksi
