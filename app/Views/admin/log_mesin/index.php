<?= view('layout/header', ['title' => 'Log Riwayat Mesin']) ?>

<div class="page-header">
  <div>
    <h1>Log Riwayat Mesin</h1>
    <p class="text-muted mb-0">Riwayat pencatatan (tambah, ubah, dan hapus) data mesin.</p>
  </div>
</div>
...
<div class="card shadow-sm mb-4">
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-hover align-middle mb-0" id="logMesinTable">
        <thead class="table-light">
          ...
          <tr class="bg-white">
            ...
            <th class="py-2 text-center align-middle">
                <a href="<?= site_url('admin/log-mesin') ?>" class="btn btn-sm btn-danger fw-bold px-3" title="Reset Filter" style="font-size: 0.75rem;">
        ...
        <tbody>
          <?php if(empty($logs)): ?>
            <tr>
              <td colspan="7" class="text-center py-4">Belum ada riwayat aktivitas mesin.</td>
            </tr>
          <?php else: ?>
            <?php $i = 1; foreach($logs as $log): ?>
              <tr>
                ...
                <td>
                  <span class="fw-bold"><?= esc($log['no_mesin']) ?></span>
                </td>
                <td>
    ...
    <form id="filterForm" action="<?= site_url('admin/log-mesin') ?>" method="get"></form>
    ...
    <div class="d-flex justify-content-between align-items-center px-3 py-2 border-top mt-2 flex-wrap gap-2">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <ul class="pagination pagination-sm mb-0 gap-1">
                ...
                <a class="page-link rounded-2" <?= $currentPage <= 1 ? 'tabindex="-1" aria-disabled="true"' : 'href="'.site_url('admin/log-mesin') . $buildQuery(['page' => $currentPage - 1]).'"' ?>>
                ...
                <li class="page-item <?= $p === $currentPage ? 'active' : '' ?>">
                    <a class="page-link rounded-2" href="<?= site_url('admin/log-mesin') . $buildQuery(['page' => $p]) ?>"><?= $p ?></a>
                </li>
                <?php endfor; ?>
                <li class="page-item <?= $currentPage >= max(1, $totalPages) ? 'disabled' : '' ?>">
                <a class="page-link rounded-2" <?= $currentPage >= max(1, $totalPages) ? 'tabindex="-1" aria-disabled="true"' : 'href="'.site_url('admin/log-mesin') . $buildQuery(['page' => $currentPage + 1]).'"' ?>>
