...
// 1. Replace streamKontrolExcel (the single export wrapper)
$oldStreamKontrolExcelStart = 'private function streamKontrolExcel(array $data): void';
$posStart1 = strpos($c, $oldStreamKontrolExcelStart);
if ($posStart1 !== false) {
    // Find the end of this function. The next function is streamKontrolAllExcel
    $posEnd1 = strpos($c, 'private function streamKontrolAllExcel(array $data): void', $posStart1);
    if ($posEnd1 !== false) {
        $newFunc1 = <<<'PHP'
...
PHP;
        $c = substr($c, 0, $posStart1) . $newFunc1 . substr($c, $posEnd1);
    }
}

// 2. Replace streamKontrolAllExcel (the massive custom builder)
$oldStreamKontrolAllExcelStart = 'private function streamKontrolAllExcel(array $data): void';
$posStart2 = strpos($c, $oldStreamKontrolAllExcelStart);
if ($posStart2 !== false) {
    // Find the end of this function (it's the last function in the class before '}')
    // Actually, we can just substr to the end of the file except the last `}`
    $posEnd2 = strrpos($c, '}');
...
PHP;
    ...
    // We replace from $posStart2 all the way to the end of the class.
    $c = substr($c, 0, $posStart2) . $newFunc2 . "\n}\n";
}

file_put_contents($f, $c);
echo "Successfully applied both streamKontrolExcel and streamKontrolAllExcel!";
