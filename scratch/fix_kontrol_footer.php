...
// Let's find the KETERANGAN CHECK LIST block and replace it up to the end of the class.
$startStr = '// KETERANGAN CHECK LIST';
$posStart = strpos($c, $startStr);

if ($posStart !== false) {
    // Find the end of streamKontrolAllExcel
    ...
    $newFooter = <<<'PHP'
            ...
            // 1. KETERANGAN CHECK LIST (Columns A-C)
            $sheet->mergeCells("A{$row}:C{$row}"); $sheet->setCellValue("A{$row}", 'KETERANGAN CHECK LIST');
            $sheet->getStyle("A{$row}:C{$row}")->getFont()->setBold(true);
            $sheet->getStyle("A{$row}:C{$row}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

            $r = $row + 1;
            $sheet->setCellValue("A{$r}", 'V'); $sheet->getStyle("A{$r}")->getFont()->setBold(true); $sheet->getStyle("A{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->setCellValue("B{$r}", ':'); $sheet->getStyle("B{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->setCellValue("C{$r}", 'OK'); $sheet->getStyle("C{$r}")->getFont()->setBold(true);

            $r++;
            $sheet->setCellValue("A{$r}", 'Δ'); $sheet->getStyle("A{$r}")->getFont()->setBold(true); $sheet->getStyle("A{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->setCellValue("B{$r}", ':'); $sheet->getStyle("B{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->setCellValue("C{$r}", 'PERLU TINDAKAN'); $sheet->getStyle("C{$r}")->getFont()->setBold(true);

            $r++;
            $sheet->setCellValue("A{$r}", 'X'); $sheet->getStyle("A{$r}")->getFont()->setBold(true); $sheet->getStyle("A{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->setCellValue("B{$r}", ':'); $sheet->getStyle("B{$r}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->setCellValue("C{$r}", 'TIDAK ADA'); $sheet->getStyle("C{$r}")->getFont()->setBold(true);

            $sheet->getStyle("A{$row}:C{$r}")->applyFromArray($borderStyle);

            // 2. SIGNATURES (Columns D-I)
            $c1 = 'D'; $c1e = 'E';
            $c2 = 'F'; $c2e = 'G';
            $c3 = 'H'; $c3e = 'I';
            ...
            $sheet->mergeCells("{$c3}{$row2}:{$c3e}{$row2}"); $sheet->setCellValue("{$c3}{$row2}", "SEC.HEAD MTC");
