...
// The new PDF-like layout helpers and builders
$newHelpers = <<<'PHP'
    ...
    private function buildPdfLikeHeader(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $sheet, array $header, $isPreventive, $isMfg2) {
        ...
        $sheet->getStyle("B2:{$lastCol}3")->getAlignment()->setHorizontal('center')->setVertical('center');
        ...
        if ($isMfg2) {
            ...
            $sheet->setCellValue('F6', 'START TIME: ' . date('H:i:s', $waktuMulai));
        } else {
        ...
        if ($isMfg2) {
            $sheet->setCellValue('F7', 'FINISH TIME: ' . ($waktuSelesai ? date('H:i:s', $waktuSelesai) : '-'));
        } else {
    ...
    private function buildPdfLikeFooter(\PhpOffice\PhpSpreadsheet\Worksheet\Worksheet $sheet, $row, $header, $isMfg2) {
        ...
        // Keterangan in A to B, Signatures in C to G
        $sheet->mergeCells("A{$row}:B{$row}");
        $sheet->setCellValue("A{$row}", 'KETERANGAN CHECK LIST');
        $sheet->getStyle("A{$row}")->getFont()->setBold(true)->setSize(9);
        $sheet->getStyle("A{$row}")->getAlignment()->setHorizontal('center');
        $rowK = $row + 1;
        $sheet->setCellValue("A{$rowK}", '√ : OK'); $sheet->setCellValue("B{$rowK}", 'Δ : PERLU TINDAKAN'); $rowK++;
        $sheet->setCellValue("A{$rowK}", 'X : TIDAK ADA');
        $sheet->getStyle("A{$row}:B".($row+4))->applyFromArray($borderOutline);
        
        $c1 = 'C'; $c2 = 'D'; $c3 = 'E'; $c4 = $isMfg2 ? 'F' : 'F';
        if (!$isMfg2) {
        ...
        $sheet->setCellValue("{$c1}{$row}", 'PREPARED'); $sheet->setCellValue("{$c2}{$row}", 'CHECKED'); $sheet->setCellValue("{$c3}{$row}", 'APPROVED'); $sheet->setCellValue("{$c4}{$row}", 'APPROVED');
        $sheet->getStyle("{$c1}{$row}:{$lastCol}{$row}")->getFont()->setBold(true)->setSize(9);
        
        $row2 = $row + 1;
        $sheet->setCellValue("{$c1}{$row2}", 'INSPECTOR'); $sheet->setCellValue("{$c2}{$row2}", 'USER'); $sheet->setCellValue("{$c3}{$row2}", 'SEC.HEAD MFG 2'); $sheet->setCellValue("{$c4}{$row2}", 'SEC.HEAD MTN');
        ...
        $rowName = $row + 5;
        $sheet->setCellValue("{$c1}{$rowName}", strtoupper($picName)); $sheet->setCellValue("{$c2}{$rowName}", strtoupper($l1Name)); $sheet->setCellValue("{$c3}{$rowName}", strtoupper($l2Name)); $sheet->setCellValue("{$c4}{$rowName}", strtoupper($finalName));
...
// Find position of last brace
$lastBrace = strrpos($c, '}');
$c = substr_replace($c, "\n\n" . $extractedFunctions . "\n\n" . $newHelpers . "\n}", $lastBrace, 1);

file_put_contents($f, $c);
echo "All methods restored and injected!";
