...
// Remove the 7-column PDF logic I just added
$c = preg_replace('/private function setupPdfLikeExcel.*?(?=\n    public function downloadExcelDetail)/s', '', $c);

// We need to insert the NEW 32-col logic right before downloadExcelDetail
$new32Col = <<<'PHP'
...
preg_match('/public function downloadExcelDetail.*?\{/s', $c, $m, PREG_OFFSET_CAPTURE);
if (!empty($m)) {
    $c = substr_replace($c, $new32Col . "\n\n", $m[0][1], 0);
} else {
    die("Could not find downloadExcelDetail to inject the new 32-col grid logic");
}

file_put_contents($f, $c);
echo "New 32-col layout restored and customized!";
